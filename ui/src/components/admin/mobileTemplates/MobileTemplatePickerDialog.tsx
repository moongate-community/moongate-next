...
import { useMobileTemplateSearch } from "../../../lib/useMobileTemplateSearch";
...
/** Reusable modal that browses mobile templates as an image grid and returns the chosen template id. */
export function MobileTemplatePickerDialog({
  ...
  const { items, loading, error, hasMore, loadMore, reset } = useMobileTemplateSearch(accessToken, {
    search,
    includeAbstract
  });
      ...
      <DialogContent className="flex max-h-[80vh] w-[60vw] max-w-[60vw] flex-col gap-3 bg-surface sm:max-w-[60vw]">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          {error ? (
            ...
            <p className="py-10 text-center text-[13px] text-fg-muted">No mobiles found.</p>
          ) : (
            <div className="grid grid-cols-3 gap-2 sm:grid-cols-4 md:grid-cols-6">
              {items.map((template) => (
                <Tooltip key={template.id}>
                  ...
                  >
                    <MobileTemplateTooltip template={template} />
                  </TooltipContent>
                </Tooltip>
              ))}
            </div>
