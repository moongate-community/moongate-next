...
export function ItemTemplateDetailPanel({ template, loading, error }: ItemTemplateDetailPanelProps) {
  if (loading) {
    return (
      <aside className="rounded-lg border border-border bg-surface p-5 text-sm font-semibold text-fg-muted shadow-card">
        Loading template...
      </aside>
    );
  }
  ...
  if (error) {
    return (
      <aside className="rounded-lg border border-danger/20 bg-danger/10 p-5 text-sm font-semibold text-danger shadow-card">
        {error}
      </aside>
    );
  }
