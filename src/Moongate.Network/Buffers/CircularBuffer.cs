...
#region IEnumerable<T> implementation
    ...
    public void PushBackRange(ReadOnlySpan<T> items)
    {
        ...
        // If we're pushing more than the whole capacity, only the last `capacity` items can survive.
        if (items.Length >= capacity)
        {
            items[^capacity..].CopyTo(_buffer);
            _start = 0;
            _end = 0;
            Size = capacity;
        ...
        while (remaining > 0)
        {
            var contiguous = Math.Min(remaining, capacity - _end);
            items.Slice(srcOffset, contiguous).CopyTo(_buffer.AsSpan(_end));

            var wasFull = IsFull;
            _end = (_end + contiguous) % capacity;
