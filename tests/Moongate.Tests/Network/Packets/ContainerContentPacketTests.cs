...
public sealed class ContainerContentPacketTests
{
    ...
    public void Write_WritesHeaderAndPerItemEntries()
    {
        ...
        Assert.Equal(0x40000001u, ((uint)bytes[19] << 24) | ((uint)bytes[20] << 16) | ((uint)bytes[21] << 8) | bytes[22]); // container id
    }
    ...
    public void Write_ClampsAmountToAtLeastOne()
    {
        var items = new List<ItemEntity>
        {
            new() { Id = new Serial(Serial.ItemOffset + 2), ItemId = 0x0EED, Amount = 0,
