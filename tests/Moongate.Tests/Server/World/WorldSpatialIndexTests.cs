...
public sealed class WorldSpatialIndexTests
{
    ...
    public void GetMobilesInRange_GathersAdjacentSectors_AndFiltersByDistance()
    {
        ...
        var alsoNear = new MobileEntity { Id = new Serial(2), MapId = 0, Location = new Point3D(14, 16, 0) };   // sector (0,1), distance 2
        ...
        var found = index.GetMobilesInRange(0, new Point3D(16, 16, 0), range: 3);
    ...
    public void GetPlayersInRange_ExcludesNpcs()
    {
        ...
        var players = index.GetPlayersInRange(0, new Point3D(50, 50, 0), range: 5);
    ...
    public void GetItemsInRange_ReturnsGroundItemsInRange()
    {
        ...
        Assert.Contains(item, index.GetItemsInRange(0, new Point3D(51, 50, 0), range: 2));
        Assert.DoesNotContain(item, index.GetItemsInRange(0, new Point3D(60, 60, 0), range: 2));
    }
    ...
    public void AddMobile_SameSerialTwice_Replaces_NoDuplicate()
    {
        ...
        var second = new MobileEntity { Id = new Serial(7), MapId = 0, Location = new Point3D(80, 80, 0) }; // different sector
        ...
        Assert.Contains(second, index.GetMobilesInRange(0, new Point3D(80, 80, 0), range: 1));
        Assert.DoesNotContain(first, index.GetMobilesInRange(0, new Point3D(10, 10, 0), range: 1)); // old bucket cleared
    }
    ...
    public void MoveMobile_WithinSameSector_UpdatesLocation_StillQueryable()
    {
        ...
        Assert.Single(index.All);
        Assert.Contains(mobile, index.GetMobilesInRange(0, new Point3D(12, 12, 0), range: 1));
    }
    ...
    public void RemoveItem_RemovesFromQueries_AndIsIdempotent()
    {
        ...
        Assert.Empty(index.GetItemsInRange(0, new Point3D(50, 50, 0), range: 2));
