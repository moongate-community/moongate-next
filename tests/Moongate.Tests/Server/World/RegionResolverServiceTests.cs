...
public sealed class RegionResolverServiceTests
{
    ...
    public void GetMusic_ReturnsResolvedRegionMusicType()
    {
        var data = new FakeRegionDataService(
            RegionWithMusic("withMusic", 0, 1, new RegionAreaEntry(0, 0, 10, 10), "Britain1")
        );
        var resolver = new RegionResolverService(data);

        Assert.Equal(MusicType.Britain1, resolver.GetMusic(0, new Point3D(5, 5, 0)));
        Assert.Equal(MusicType.NoMusic, resolver.GetMusic(0, new Point3D(999, 999, 0)));
    }
    ...
    {
        return new RegionEntry("BaseRegion", mapId, "Map", name, priority, new[] { area }, music, null, null);
    }
