...
public sealed class InterestManagementServiceTests
{
    ...
    private static (InterestManagementService svc, WorldSpatialIndex index, RecordingOutgoingPacketQueue outgoing,
        PlayerSessionService sessions, MobileEntity viewer)
        Build()
    {
    ...
    public async Task SendInitialSnapshot_SendsNearbyMobilesAndItems_AndSeedsKnownSet()
    {
        ...
        { Id = new Serial(11), MapId = 0, Location = new Point3D(101, 100, 0) }; // dist 1, in range
        var far = new MobileEntity
        { Id = new Serial(12), MapId = 0, Location = new Point3D(100, 150, 0) }; // dist 50, out of range (>24)
        ...
        Assert.DoesNotContain(
            outgoing.Sent,
            s => s.Packet is MobileIncomingPacket p && p.Mobile.Id == far.Id
        ); // out of range
        Assert.DoesNotContain(
            outgoing.Sent,
            s => s.Packet is MobileIncomingPacket p && p.Mobile.Id == viewer.Id
        ); // never self
    }
    ...
    public async Task OnEntityRemoved_SendsDeleteToObserversThatKnewIt()
    {
        ...
        Assert.Contains(
            outgoing.Sent,
            s => s.SessionId == ViewerSession && s.Packet is DeleteObjectPacket d && d.Serial == other.Id
        );
    }
    ...
    public async Task OnMobileMoved_ObserverGainsThenLosesSight()
    {
        ...
        var mover = new MobileEntity
        { Id = new Serial(20), MapId = 0, Location = new Point3D(100, 130, 0) }; // out of range (30 > 24)
        ...
        await svc.OnMobileMovedAsync(
            new MobileMovedEvent(mover.Id, 0, new Point3D(100, 130, 0), new Point3D(100, 124, 0), DirectionType.North)
        );
        Assert.Contains(
            outgoing.Sent,
            s => s.SessionId == ViewerSession && s.Packet is MobileIncomingPacket p && p.Mobile.Id == mover.Id
        );
        outgoing.Sent.Clear();
        ...
        await svc.OnMobileMovedAsync(
            new MobileMovedEvent(mover.Id, 0, new Point3D(100, 124, 0), new Point3D(100, 123, 0), DirectionType.North)
        );
        Assert.Contains(
            outgoing.Sent,
            s => s.SessionId == ViewerSession && s.Packet is MobileMovingPacket p && p.Mobile.Id == mover.Id
        );
        outgoing.Sent.Clear();
        ...
        await svc.OnMobileMovedAsync(
            new MobileMovedEvent(mover.Id, 0, new Point3D(100, 123, 0), new Point3D(100, 130, 0), DirectionType.South)
        );
        Assert.Contains(
            outgoing.Sent,
            s => s.SessionId == ViewerSession && s.Packet is DeleteObjectPacket d && d.Serial == mover.Id
        );
    }
    ...
    public async Task OnMobileMoved_MoverSide_SeesEntitiesEnterAndLeaveView()
    {
        ...
        var target = new MobileEntity
        { Id = new Serial(30), MapId = 0, Location = new Point3D(100, 140, 0) }; // dist 40, out of range (non-player)
        var item = new ItemEntity
        { Id = new Serial(0x40000010), MapId = 0, Location = new Point3D(100, 141, 0) }; // dist 41, out of range
        ...
        await svc.OnMobileMovedAsync(
            new MobileMovedEvent(viewer.Id, 0, new Point3D(100, 100, 0), new Point3D(100, 120, 0), DirectionType.South)
        );
        Assert.Contains(
            outgoing.Sent,
            s => s.SessionId == ViewerSession && s.Packet is MobileIncomingPacket p && p.Mobile.Id == target.Id
        );
        Assert.Contains(
            outgoing.Sent,
            s => s.SessionId == ViewerSession && s.Packet is ObjectInformationPacket p && p.Item.Id == item.Id
        );
        outgoing.Sent.Clear();
        ...
        await svc.OnMobileMovedAsync(
            new MobileMovedEvent(viewer.Id, 0, new Point3D(100, 120, 0), new Point3D(100, 100, 0), DirectionType.North)
        );
        Assert.Contains(
            outgoing.Sent,
            s => s.SessionId == ViewerSession && s.Packet is DeleteObjectPacket d && d.Serial == target.Id
        );
        Assert.Contains(
            outgoing.Sent,
            s => s.SessionId == ViewerSession && s.Packet is DeleteObjectPacket d && d.Serial == item.Id
        );
    }
}
