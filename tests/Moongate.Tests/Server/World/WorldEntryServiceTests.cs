...
public sealed class WorldEntryServiceTests
{
    ...
    {
        return new InterestManagementService(new WorldSpatialIndex(), outgoing, new PlayerSessionService(), items);
    }
    ...
    public async Task EnterWorldAsync_SendsRegionMusic()
    {
        ...
        var region = new RegionEntry(
            ...
            "Britain",
            1,
            new[] { new RegionAreaEntry(0, 0, 100, 100) },
            "Britain1",
            null,
            null
        );
...
internal sealed class FakeMapService : IMapService
{
    ...
    {
        return _map is not null && _map.MapId == mapId ? _map : null;
    }
}
