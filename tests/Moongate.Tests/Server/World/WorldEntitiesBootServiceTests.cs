...
public sealed class WorldEntitiesBootServiceTests
{
    ...
    public async Task StartAsync_AddsNpcsAndGroundItems_SkipsPlayersAndContainedItems()
    {
        ...
        var containedItem = new ItemEntity { Id = new Serial(0x40000002), MapId = 0, Location = new Point3D(51, 50, 0), ParentContainerId = new Serial(0x40000099) };
        ...
        Assert.DoesNotContain(containedItem, index.GetItemsInRange(0, new Point3D(51, 50, 0), 1)); // contained item NOT added
    }
        ...
        where TEntity : class
    {
        ...
        public ValueTask<IReadOnlyCollection<TEntity>> GetAllAsync(CancellationToken cancellationToken = default)
            => ValueTask.FromResult(_all);

        public ValueTask<int> CountAsync(CancellationToken cancellationToken = default) => throw new NotSupportedException();
        public ValueTask<TEntity?> GetByIdAsync(Serial id, CancellationToken cancellationToken = default) => throw new NotSupportedException();
        ...
        public ValueTask UpsertAsync(TEntity entity, CancellationToken cancellationToken = default) => throw new NotSupportedException();
        public ValueTask<Serial> NextIdAsync(CancellationToken cancellationToken = default) => throw new NotSupportedException();
    }
}
