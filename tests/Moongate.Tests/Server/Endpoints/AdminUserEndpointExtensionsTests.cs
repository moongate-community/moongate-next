...
public sealed class AdminUserEndpointExtensionsTests
{
    private static ClaimsPrincipal Caller(string id)
        => new(new ClaimsIdentity([new Claim(ClaimTypes.NameIdentifier, id)], "jwt"));
    ...
    public async Task HandleDeleteAsync_SelfDelete_ReturnsForbidden()
    {
        ...
        var result = await AdminUserEndpointExtensions.HandleDeleteAsync(service, Caller("0x00000001"), "0x00000001", CancellationToken.None);
    ...
    public async Task HandleDeleteAsync_OtherUser_ReturnsNoContent()
    {
        ...
        var result = await AdminUserEndpointExtensions.HandleDeleteAsync(service, Caller("0x00000001"), "0x00000002", CancellationToken.None);
    ...
    public async Task HandleSetActiveAsync_LockSelf_ReturnsForbidden()
    {
        ...
        var result = await AdminUserEndpointExtensions.HandleSetActiveAsync(
            service, Caller("0x00000001"), "0x00000001", new SetUserActiveRequest { IsActive = false }, CancellationToken.None);
    ...
    public async Task HandleUpdateAsync_UnknownUser_ReturnsNotFound()
    {
        ...
        var result = await AdminUserEndpointExtensions.HandleUpdateAsync(
            service, Caller("0x00000001"), "0x00000099",
            new UpdateUserRequest { Email = "x@x.local", Level = "Player" }, CancellationToken.None);
    ...
    private sealed class FakeUserService : IUserService
    {
        ...
        public void Seed(UserEntity user) => _users[user.Id] = user;

        public ValueTask<PagedResult<UserEntity>> ListAsync(PageRequest request, CancellationToken cancellationToken = default)
        {
        ...
        public ValueTask<UserEntity> CreateAsync(string username, string email, string password,
            UserLevelType level = UserLevelType.Player, bool isActive = true, CancellationToken cancellationToken = default)
        {
            if (ThrowConflict)
            {
        ...
        public ValueTask<UserEntity?> GetByUsernameAsync(string username, CancellationToken cancellationToken = default)
            => ValueTask.FromResult(_users.Values.FirstOrDefault(u =>
                string.Equals(u.Username, username, StringComparison.OrdinalIgnoreCase)));

        public ValueTask<UserEntity?> UpdateAsync(Serial id, string email, UserLevelType level, CancellationToken cancellationToken = default)
        {
