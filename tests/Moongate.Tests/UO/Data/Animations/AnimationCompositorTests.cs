...
public sealed class AnimationCompositorTests
{
    ...
    public void Compose_AlignsByCentre_AndOverlaysOpaquePixels()
    {
        ...
        for (var y = 0; y < 2; y++)
        {
            for (var x = 0; x < 2; x++)
            {
                baseImg[x, y] = new(255, 0, 0, 255);
            }
        }

        // overlay: 1x1 green, centre (520,520), height 1 -> top-left (-8, 512-520-1) = (-8,-9)
        var overlay = new Image<Rgba32>(1, 1);
        overlay[0, 0] = new(0, 255, 0, 255);

        using var result = AnimationCompositor.Compose([new(baseImg, 520, 520), new(overlay, 520, 520)]);

        Assert.Equal(2, result.Width);
        Assert.Equal(2, result.Height);
        Assert.Equal(new(0, 255, 0, 255), result[0, 1]); // overlay landed here
        Assert.Equal(new(255, 0, 0, 255), result[0, 0]); // base elsewhere
        Assert.Equal(new(255, 0, 0, 255), result[1, 1]);
    }
