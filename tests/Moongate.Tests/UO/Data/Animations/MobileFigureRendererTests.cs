...
public sealed class MobileFigureRendererTests
{
    ...
    {
        return new EquipConvTable(Path.Combine(Path.GetTempPath(), $"noequip-{Guid.NewGuid():N}.def"));
    }
    ...
    private sealed class FakeTileData : ITileDataStore
    {
        ...
        {
            return new ItemData { Animation = _hairAnim.GetValueOrDefault(id) };
        }
    ...
    private sealed class FakeItems : IItemTemplateService
    {
        ...
        {
            _map[id] = new ItemTemplateDefinition { Id = id, ItemId = itemId, Layer = layer, Hue = hue };
        }
    }
}
